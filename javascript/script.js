...
// imperial
const feetErrorMessage = document.getElementById('error-message-feet');
const ft = document.getElementById('ft');
const inchErrorMessage = document.getElementById('error-message-inch');
const inch = document.getElementById('inch');

const stoneErrorMessage = document.getElementById('error-message-st');
const st = document.getElementById('st');
const lbsErrorMessage = document.getElementById('error-message-lbs');
...
// === IMPERIAL HEIGHT ERROR STATES === //
// feet
feetInput.addEventListener('input', () => {
    const typedInValueAsNum = Number(feetInput.value);

    if (feetInput.value === '' || isNaN(typedInValueAsNum) || typedInValueAsNum < 0 || typedInValueAsNum > 9) {
        feetErrorMessage.style.display = 'block';
        feetInput.style.border = '1px solid red';
        ft.style.color = 'red';
    } 
    
    else {
        feetErrorMessage.style.display = 'none';
        feetInput.style.border = '';
        ft.style.color = '';
    }
});
...
inchInput.addEventListener('input', () => {
    const typedInValueAsNum = Number(inchInput.value);

    if (inchInput.value === '' || isNaN(typedInValueAsNum) || typedInValueAsNum < 0 || typedInValueAsNum > 11) {
        inchErrorMessage.style.display = 'block';
        inchInput.style.border = '1px solid red';
        inch.style.color = 'red';
    } 
    
    else {
        inchErrorMessage.style.display = 'none';
        inchInput.style.border = '';
        inch.style.color = '';
    }
});

// === IMPERIAL WEIGHT ERROR STATES === //
// st
stInput.addEventListener('input', () => {
    const typedInValueAsNum = Number(stInput.value);

    if (stInput.value === '' || isNaN(typedInValueAsNum) || typedInValueAsNum < 0 || typedInValueAsNum > 50) {
        stoneErrorMessage.style.display = 'block';
        stInput.style.border = '1px solid red';
        st.style.color = 'red';
    } 
    
    else {
        stoneErrorMessage.style.display = 'none';
        stInput.style.border = '';
        st.style.color = '';
    }
});
...
lbsInput.addEventListener('input', () => {
    const typedInValueAsNum = Number(lbsInput.value);

    if (lbsInput.value === '' || isNaN(typedInValueAsNum) || typedInValueAsNum < 0 || typedInValueAsNum > 13) {
        lbsErrorMessage.style.display = 'block';
        lbsInput.style.border = '1px solid red';
        lbs.style.color = 'red';
    } 
    
    else {
        lbsErrorMessage.style.display = 'none';
        lbsInput.style.border = '';
        lbs.style.color = '';
    }
});
...
=== We need to add value = '0' to those input fields in html again if we want to use this function ===
*/
...
function calculateImperialBmi() {
    ...
    if (imperialButton.checked) {
      ...
      // === IF THERE IS AN INPUT FIELD THE DEFAULT TEXT SHOWS UP ONLY === //  
        if (feetInput.value.trim() === '' || inchInput.value.trim() === '' || stInput.value.trim() === '' || lbsInput.value.trim() === '') {
            resultHeader.textContent = defaultHeaderText;
            resultParagraph.innerHTML = defaultParagraphText;
            longerSuggestions.innerHTML = defaultLongSuggestionText;
            return;
        ...
        const st = Number(stInput.value);
        const lbs = Number(lbsInput.value);

        if (
            isNaN(feet) || isNaN(inch) ||
            isNaN(st) || isNaN(lbs) ||
            feet < 0 || feet > 9 ||
            inch < 0 || inch > 11 ||
            st < 0 || st > 50 ||
        ...
        const totalWeightInPounds = (st * 14) + lbs;

        const bmi = ((totalWeightInPounds * 703) / (totalHeightInInches ** 2)).toFixed(2);
...
// === REAL TIME IMPERIAL CALCULATION === //
feetInput.addEventListener('input', calculateImperialBmi);
inchInput.addEventListener('input', calculateImperialBmi);
stInput.addEventListener('input', calculateImperialBmi);
lbsInput.addEventListener('input', calculateImperialBmi);
